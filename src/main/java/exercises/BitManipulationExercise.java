...
public class BitManipulationExercise {
  ...
  public static class MonochromeScreen {
    ...
    public byte[] drawHorizontalLine(final int startColumn, final int endColumn, final int row) {
      if (row < 0 || row >= height) {
        throw new IllegalArgumentException("Invalid row index.");
      }
      if (startColumn < 0 || startColumn >= width
          || endColumn < 0 || endColumn >= width || startColumn > endColumn) {
        throw new IllegalArgumentException("Invalid startColumn or endColumn values.");
      }

      final int byteRowStartIndex = (row * width) / 8;

      for (int columnIndex = startColumn; columnIndex <= endColumn; columnIndex++) {
        final int byteOffset = byteRowStartIndex + (columnIndex  / 8);
        final int bitPosition = columnIndex  % 8;
