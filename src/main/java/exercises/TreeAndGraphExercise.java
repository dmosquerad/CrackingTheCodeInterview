...
public class TreeAndGraphExercise {
  ...
  public static class GraphWithDistance {
      ...
      while (!priorityQueue.isEmpty()) {
        ...
        final Set<Node> unvisitedNeighbours = currentNode.getNeighbours().stream()
            .filter(neighbour -> priorityQueue.stream()
                .noneMatch(priorityNeighbour -> Objects.equals(priorityNeighbour.getNode(), neighbour)))
    ...
    private static boolean hasRouteAstarLambda(final @NonNull Node start, final @NonNull Node end,
      ...
      final Map<GraphWithDistance.Node, Integer> pathCosts = new HashMap<>();
      pathCosts.put(start, 0);
  ...
  public static class EuclideanHeuristic implements Heuristic {
    @Override
    public int heuristic(final @NonNull GraphWithDistance.Node current, final @NonNull GraphWithDistance.Node goal) {
  ...
  public static class TreeNodeExercise {
    ...
    // Exercise 4.3
    public static Collection<LinkedList<TreeNode>> createLevelLinkedList(final @NonNull TreeNode root) {
      return createLevelLinkedList(root, new ArrayList<>(), 0);
    }

    private static List<LinkedList<TreeNode>> createLevelLinkedList(final @NonNull TreeNode currentTree,
    ...
    public static Collection<LinkedList<TreeNode>> createLevelLinkedListLambda(final @NonNull TreeNode root) {
    ...
    private static BalanceStatus checkHeight(final @NonNull TreeNode treeNode) {
      final BalanceStatus leftBalanced = Objects.nonNull(treeNode.getLeft()) ? checkHeight(treeNode.getLeft()) : BalanceStatus.BALANCED;
      final BalanceStatus rightBalanced = Objects.nonNull(treeNode.getRight()) ? checkHeight(treeNode.getRight()) : BalanceStatus.BALANCED;

      if (BalanceStatus.UNBALANCED.equals(leftBalanced) || BalanceStatus.UNBALANCED.equals(rightBalanced)) {
        return BalanceStatus.UNBALANCED;
      }

      final int leftHeight = Objects.nonNull(treeNode.getLeft()) ? getHeight(treeNode.getLeft()) : 0;
      final int rightHeight = Objects.nonNull(treeNode.getRight()) ? getHeight(treeNode.getRight()) : 0;
    ...
    private static int checkHeightInt(final @NonNull TreeNode treeNode) {
      final int leftHeight = Objects.nonNull(treeNode.getLeft()) ? checkHeightInt(treeNode.getLeft()) : 0;
      final int rightHeight = Objects.nonNull(treeNode.getRight()) ? checkHeightInt(treeNode.getRight()) : 0;
    ...
    private static boolean isValidBST(final @NonNull TreeNode treeNode, final int minimum, final int maximum) {
      if (treeNode.getValue() <= minimum || treeNode.getValue() >= maximum) {
        return false;
      }

      final boolean isValidLeftBTS = Objects.isNull(treeNode.getLeft()) || isValidBST(treeNode.getLeft(), minimum, treeNode.getValue());
      final boolean isValidRightBTS = Objects.isNull(treeNode.getRight()) || isValidBST(treeNode.getRight(), treeNode.getValue(), maximum);

      return isValidLeftBTS && isValidRightBTS;
    }

    // Exercise 4.6
    public static Optional<TreeNode> findInOrderSuccessor(final @NonNull TreeNode root, final @NonNull TreeNode treeNode) {
    ...
    // Exercise 4.10
    public static boolean isSubtree(final @NonNull TreeNode treeNode, final @NonNull TreeNode subTreeNode) {
      if (areIdentical(Optional.of(treeNode),Optional.of(subTreeNode))) {
        return true;
      }

      final boolean treeNodeLeft = Objects.nonNull(treeNode.getLeft()) && isSubtree(treeNode.getLeft(), subTreeNode);
      final boolean treeNodeRight = Objects.nonNull(treeNode.getRight()) && isSubtree(treeNode.getRight(), subTreeNode);

      return treeNodeLeft || treeNodeRight;
    }

    private static boolean areIdentical(final @NonNull Optional<TreeNode> treeNode,
                                        final @NonNull Optional<TreeNode> subTreeNode) {
      if (treeNode.isEmpty() && subTreeNode.isEmpty()) return true; // Both are null, they are identical
      if (treeNode.isEmpty() || subTreeNode.isEmpty()) return false; // One is null, they can't be identical

      return (treeNode.get().getValue() == subTreeNode.get().getValue())
              && areIdentical(Optional.ofNullable(treeNode.get().getLeft()),
                Optional.ofNullable(subTreeNode.get().getLeft()))
              && areIdentical(Optional.ofNullable(treeNode.get().getRight()),
                Optional.ofNullable(subTreeNode.get().getRight()));
    }

    public static boolean isSubtreeLambda(final @NonNull TreeNode treeNode, final @NonNull TreeNode subTreeNode) {
      return Optional.of(treeNode)
              .filter(node -> areIdenticalLambda(Optional.of(node), Optional.of(subTreeNode)))
              .map(n -> true)
              .orElseGet(() ->
                      Stream.of(
                              Optional.ofNullable(treeNode.getLeft())
                                      .filter(left -> isSubtree(left, subTreeNode)),
                              Optional.ofNullable(treeNode.getRight())
                                      .filter(right -> isSubtree(right, subTreeNode))
                      ).anyMatch(Optional::isPresent)
              );
    }

    private static boolean areIdenticalLambda(final Optional<TreeNode> treeNode, final Optional<TreeNode> subTreeNode) {
      return (treeNode.isEmpty() && subTreeNode.isEmpty()) ||
              treeNode.flatMap(n1 -> subTreeNode
              .filter(n2 -> Objects.equals(n1.getValue(), n2.getValue()))
              .filter(n2 ->
                      areIdentical(Optional.ofNullable(n1.getLeft()), Optional.ofNullable(n2.getLeft())) &&
                              areIdentical(Optional.ofNullable(n1.getRight()), Optional.ofNullable(n2.getRight()))
              )
      ).isPresent();
    }
    ...
    public static int countPaths(final @NonNull TreeNode root, final int targetSum) {
      final int leftPath = Objects.nonNull(root.getLeft()) ? countPaths(root.getLeft(), targetSum) : 0;
      final int rightPath = Objects.nonNull(root.getRight()) ? countPaths(root.getRight(), targetSum) : 0;
  ...
  // Exercise 4.7
  private static Set<Character> gatherAllDependencies(final @NonNull HashMap<Character, Collection<Character>> project){
  ...
  private static Map<Character, Integer> initializeDependencyCalls(
          final @NonNull HashMap<Character, Collection<Character>> project,
    ...
    for (Map.Entry<Character, Collection<Character>> entry : project.entrySet()) {
      ...
      entry.getValue().forEach(i -> dependencyCalls.put(dependency, dependencyCalls.get(dependency) + 1));
    }

    return dependencyCalls;
  }

  private static Queue<Character> findInitialDependencies(final @NonNull Map<Character, Integer> dependencyCalls) {
  ...
  private static List<Character> resolveDependencies(final @NonNull Map<Character, List<Character>> graphDependencies,
  ...
  public static Collection<Character> dependencyOrder(
    ...
    final Map<Character, Integer> dependencyCalls = initializeDependencyCalls(project, allDependencies);

    final Queue<Character> initialDependency = findInitialDependencies(dependencyCalls);

    final Map<Character, List<Character>> graphDependencies = buildGraph(project, allDependencies);

    final List<Character> dependencyOrder = resolveDependencies(graphDependencies, dependencyCalls, initialDependency);
  ...
  private static Queue<Character> findInitialDependenciesLambda(final @NonNull Map<Character, Integer> dependencyCalls) {
  ...
  private static void checkForCyclesLambda(final @NonNull Set<Character> allDependencies,
                                     final @NonNull List<Character> dependencyOrder) {
    final Set<Character> circularDependencies = allDependencies.stream()
            .filter(dependency -> !dependencyOrder.contains(dependency))
            .collect(Collectors.toUnmodifiableSet());
  ...
  public static Collection<Character> dependencyOrderLambda(
    ...
    final Map<Character, Integer> dependencyCalls = initializeDependencyCallsLambda(project, allDependencies);

    final Queue<Character> initialDependency = findInitialDependenciesLambda(dependencyCalls);

    final Map<Character, List<Character>> graphDependencies = buildGraphLambda(project, allDependencies);

    final List<Character> dependencyOrder = resolveDependenciesLambda(graphDependencies, dependencyCalls, initialDependency);

    checkForCyclesLambda(allDependencies, dependencyOrder);
  ...
  public static class RandomNode {
    ...
    public Optional<TreeNode> find(final int value) {
      if (value < this.root.getValue()) {
        return Objects.nonNull(this.root.getLeft()) ? this.find(this.root.getLeft().getValue()) : Optional.empty();
      }
      if (value > this.root.getValue()) {
        return Objects.nonNull(this.root.getRight()) ? this.find(this.root.getRight().getValue()) : Optional.empty();
      }
    ...
    private TreeNode insert(@NonNull TreeNode node, int value) {
      ...
      if (value < node.getValue()) {
        node = node.toBuilder()
                .left(Objects.isNull(node.getLeft()) ? TreeNode.builder().value(value).build() :
      ...
      if (value > node.getValue()) {
        node = node.toBuilder()
                .right(Objects.isNull(node.getRight()) ? TreeNode.builder().value(value).build() :
                        insert(node.getRight(), value))
                .build();
      }

      node.toBuilder().size(1 +
    ...
    private TreeNode delete(@NonNull TreeNode node, final int val) {
    ...
    public Optional<TreeNode> getRandomNode() {
      return getRandomNode(this.root, new Random().nextInt(this.root.getSize()));
    }

    private Optional<TreeNode> getRandomNode(final @NonNull TreeNode node, int randomIndex) {
      ...
      if (randomIndex < leftSize) {
        return Objects.nonNull(node.getLeft()) ? getRandomNode(node.getLeft(), randomIndex) : Optional.empty();
      }

      return Objects.nonNull(node.getRight()) ? getRandomNode(node.getRight(), randomIndex) : Optional.empty();
    }
